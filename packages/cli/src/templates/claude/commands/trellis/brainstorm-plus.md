...
- a clean handoff to `/fusion:write-task-plan`
...
6. **Plan is a separate artifact**
   Do not put the execution breakdown into `prd.md` or `info.md`. Hand off to `/fusion:write-task-plan`.
...
Next step: run /fusion:write-task-plan to generate a TDD-first execution plan in this task.
```
...
Next step: run /fusion:write-task-plan.
```
